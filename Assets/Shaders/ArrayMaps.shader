...
Shader "SLZ/Texture Arrays"
{
	Properties
	{
		...
		[Gamma][NoScaleOffset]_MetallicGlossMapArray("Metallic Gloss Map Array", 2DArray ) = "" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature _VERTEXLIT_ON
			...
			uniform UNITY_DECLARE_TEX2DARRAY( _BumpMapArray );
			uniform float _DetailNormalMapScale;
			uniform sampler2D _DetailNormalMap;
			uniform sampler2D _MetallicGlossMap;
			uniform UNITY_DECLARE_TEX2DARRAY( _MetallicGlossMapArray );
			...
			fixed4 frag (v2f i , bool bIsFrontFace : SV_IsFrontFace) : SV_Target
			{
				...
				float3 uv0_MainTex = i.ase_texcoord1.xyz;
				uv0_MainTex.xy = i.ase_texcoord1.xyz.xy * _MainTex_ST.xy + _MainTex_ST.zw;
				float4 texArray3 = UNITY_SAMPLE_TEX2DARRAY(_TextureArray, float3(uv0_MainTex.xy, uv0_MainTex.z)  );
				#ifdef _MAINARRAY_ON
				float4 staticSwitch11 = texArray3;
				#else
				float4 staticSwitch11 = tex2D( _MainTex, uv0_MainTex.xy );
				#endif
				float2 uv_DetailAlbedoMap = i.ase_texcoord1.xyz.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				
				float3 texArray8 = UnpackNormal( UNITY_SAMPLE_TEX2DARRAY(_BumpMapArray, float3(uv0_MainTex.xy, uv0_MainTex.z)  ) );
				#ifdef _BUMPARRAY_ON
				float3 staticSwitch12 = texArray8;
				#else
				float3 staticSwitch12 = UnpackNormal( tex2D( _BumpMap, uv0_MainTex.xy ) );
				#endif
				float2 uv0_DetailAlbedoMap = i.ase_texcoord1.xyz.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float3 temp_output_32_7 = BlendNormals( staticSwitch12 , UnpackScaleNormal( tex2D( _DetailNormalMap, uv0_DetailAlbedoMap ), _DetailNormalMapScale ) );
				
				float4 texArray19 = UNITY_SAMPLE_TEX2DARRAY(_MetallicGlossMapArray, float3(uv0_MainTex.xy, uv0_MainTex.z)  );
				#ifdef _METALLICGLOSSARRAY_ON
				float4 staticSwitch13 = texArray19;
				#else
				float4 staticSwitch13 = tex2D( _MetallicGlossMap, uv0_MainTex.xy );
				#endif
				float4 break23 = staticSwitch13;
				float3 temp_cast_9 = (break23.r).xxx;
				
				float3 temp_output_1_0_g6 = temp_output_32_7;
				float2 temp_output_5_0_g6 = (temp_output_1_0_g6).xy;
				float2 temp_output_2_0_g6 = abs( ( temp_output_5_0_g6 * temp_output_5_0_g6 ) );
				float lerpResult36 = lerp( 1.0 , ( ( 1.0 - ( (temp_output_2_0_g6).x + (temp_output_2_0_g6).y ) ) * (temp_output_1_0_g6).z ) , _NormalToOcclusion);
				#ifdef _VERTEXLIT_ON
				float staticSwitch30 = ( break23.g * i.ase_color.r * lerpResult36 );
				#else
				float staticSwitch30 = break23.g;
				#endif
			...
			//	float3 vNormalTs = float3( 0.0, 0.0, 1.0 );		
				...
				float3 Albedo = ( ( staticSwitch11 * _Color ) * ( tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap ) * unity_ColorSpaceDouble ) ).xyz;
				float3 Normal = temp_output_32_7;  //vNormalTs
				float3 Emission = fixed3(0,0,0);
				float Metallic = temp_cast_9;
				float Roughness = 1 - saturate( break23.b );
				float Retroreflective = 0;
				float AnisotropicDirection = 0;
				float AnisotropicRatio = 1;
				float DiffuseOcclusion = staticSwitch30;
				float SpecularOcclusion = staticSwitch30;
				float Alpha = 1;
				float Cutoff = 1;
...
/*ASEBEGIN
Version=16900
21;21;1417;1030;381.1102;468.7233;1.176199;True;True
Node;AmplifyShaderEditor.TextureCoordinatesNode;1;-1240.207,63.45738;Float;False;0;16;3;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TextureArrayNode;3;-523.8327,-107.3066;Float;True;Property;_TextureArray;TextureArray;0;1;[NoScaleOffset];Create;True;0;0;False;0;None;0;Object;-1;Auto;False;7;6;SAMPLER2D;;False;0;FLOAT2;0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;1;False;4;FLOAT2;0,0;False;5;FLOAT2;0,0;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SamplerNode;16;-516.6295,-309.2049;Float;True;Property;_MainTex;MainTex;6;0;Create;True;0;0;False;0;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.SamplerNode;18;-509.7413,130.8633;Float;True;Property;_BumpMap;_BumpMap;5;2;[NoScaleOffset];[Normal];Create;True;0;0;False;0;None;None;True;0;False;bump;Auto;True;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.TextureArrayNode;8;-518.918,351.6488;Float;True;Property;_BumpMapArray;Bump Map Array;1;1;[NoScaleOffset];Create;True;0;0;False;0;None;0;Object;-1;Auto;True;7;6;SAMPLER2D;;False;0;FLOAT2;0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;1;False;4;FLOAT2;0,0;False;5;FLOAT2;0,0;False;5;FLOAT3;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.ColorNode;38;-60.00806,-316.9936;Float;False;Property;_Color;Color;15;0;Create;True;0;0;False;0;1,1,1,1;1,1,1,1;True;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.StaticSwitch;11;-85.00867,-94.40415;Float;False;Property;_MainArray;Main Array;3;0;Create;True;0;0;False;0;0;0;1;True;;Toggle;2;Key0;Key1;Create;False;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;37;209.3414,-185.2593;Float;False;2;2;0;COLOR;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.StaticSwitch;12;-75.32077,209.0542;Float;False;Property;_BumpArray;Bump Array;7;0;Create;True;0;0;False;0;0;0;1;True;;Toggle;2;Key0;Key1;Create;False;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.TextureArrayNode;19;-530.2,821.1553;Float;True;Property;_MetallicGlossMapArray;Metallic Gloss Map Array;2;2;[Gamma];[NoScaleOffset];Create;True;0;0;False;0;None;0;Object;-1;Auto;False;7;6;SAMPLER2D;;False;0;FLOAT2;0,0;False;1;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;1;False;4;FLOAT2;0,0;False;5;FLOAT2;0,0;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.FunctionNode;32;256.3344,78.41216;Float;False;Detail Maps;11;;5;7f7bf8e1d76e0dc4fb049018978c12cf;0;2;8;FLOAT4;0,0,0,0;False;9;FLOAT3;0,0,0;False;2;FLOAT4;0;FLOAT3;7
Node;AmplifyShaderEditor.SamplerNode;20;-521.0232,602.2208;Float;True;Property;_MetallicGlossMap;Metallic Gloss Map;4;2;[Gamma];[NoScaleOffset];Create;False;0;0;False;0;None;None;True;0;False;white;Auto;False;Object;-1;Auto;Texture2D;6;0;SAMPLER2D;;False;1;FLOAT2;0,0;False;2;FLOAT;0;False;3;FLOAT2;0,0;False;4;FLOAT2;0,0;False;5;FLOAT;1;False;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.RangedFloatNode;34;377.9035,270.1535;Float;False;Property;_NormalToOcclusion;Normal To Occlusion;10;0;Create;True;0;0;False;0;1;1;0;2;0;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;33;466.7264,194.7637;Float;False;Normal to AO;-1;;6;8de3130483a168e45ac6c4f4fe4e486d;0;1;1;FLOAT3;0,0,0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;13;-120.4315,597.1008;Float;False;Property;_MetallicGlossArray;Metallic Gloss Array;8;0;Create;True;0;0;False;0;0;0;1;True;;Toggle;2;Key0;Key1;Create;False;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.BreakToComponentsNode;23;222.0198,441.7917;Float;False;COLOR;1;0;COLOR;0,0,0,0;False;16;FLOAT;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT;5;FLOAT;6;FLOAT;7;FLOAT;8;FLOAT;9;FLOAT;10;FLOAT;11;FLOAT;12;FLOAT;13;FLOAT;14;FLOAT;15
Node;AmplifyShaderEditor.VertexColorNode;31;281.7274,612.3475;Float;False;0;5;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.LerpOp;36;711.9035,197.1535;Float;False;3;0;FLOAT;1;False;1;FLOAT;1;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;29;565.6467,540.7499;Float;False;3;3;0;FLOAT;0;False;1;FLOAT;0;False;2;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;30;762.7274,475.3475;Float;False;Property;_VERTEXLIT;Vertex AO;9;0;Create;False;0;0;False;0;0;0;1;True;;Toggle;2;Key0;Key1;Create;False;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;28;1114.283,69.20137;Float;False;True;2;Float;ASEMaterialInspector;0;11;SLZ/Texture Arrays;1f6ac94e27bd0934ab97faa6217ad58e;True;VRBase;0;0;VRBase;15;False;False;False;False;False;False;False;False;False;True;1;RenderType=Opaque=RenderType;False;0;True;0;1;False;-1;0;False;-1;0;1;False;-1;0;False;-1;True;0;False;-1;0;False;-1;True;False;True;0;False;-1;True;True;True;True;True;0;False;-1;True;False;255;False;-1;255;False;-1;255;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;7;False;-1;1;False;-1;1;False;-1;1;False;-1;True;1;False;-1;True;3;False;-1;True;True;0;False;-1;0;False;-1;True;2;LightMode=ForwardBase;PassFlags=OnlyDirectional;True;2;0;;0;0;Standard;0;0;1;True;False;15;0;FLOAT3;0,0,0;False;1;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT3;0,0,0;False;8;FLOAT;0;False;9;FLOAT;0;False;10;FLOAT;0;False;11;FLOAT;0;False;12;FLOAT3;0,0,0;False;13;FLOAT4;0,0,0,0;False;14;FLOAT3;0,0,0;False;0
...
WireConnection;32;8;37;0
WireConnection;32;9;12;0
WireConnection;20;1;1;0
WireConnection;33;1;32;7
...
WireConnection;28;1;32;7
WireConnection;28;3;23;0
WireConnection;28;4;23;2
WireConnection;28;8;30;0
WireConnection;28;9;30;0
ASEEND*/
//CHKSM=E71EA929448ADC1122DDBF2D4059F1E8EAEC9E34